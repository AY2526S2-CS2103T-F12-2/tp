...
5. **Enhance `TimeSlot` to support flexible scheduling across a day boundary**: Currently, the `TimeSlot` used for `Meeting` and `Person` only accepts a time range within a single calendar day. We plan to extend `TimeSlot` parsing and validation so users can express more flexible schedules in one model, including overnight ranges that cross midnight.
